...
class MaskableRecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        assert not (self.shared_lstm
                    and self.enable_critic_lstm), "You must choose between shared LSTM, seperate or no LSTM for the critic"
    ...
    def forward(
        ...
        :param action_masks: action mask
        ...
        if action_masks is not None:
            action_masks = self._weight_to_mask(action_masks)
            distribution.apply_masking(action_masks)
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        return actions, values, log_prob, RNNStates(lstm_states_pi, lstm_states_vf)

    def _weight_to_mask(self, action_masks):
        if isinstance(action_masks, th.Tensor):
            action_masks = action_masks.numpy()
            action_masks = np.where(action_masks > 0, 1, 0)
            return th.from_numpy(action_masks)
        else:
            action_masks = np.where(action_masks > 0, 1, 0)
            return action_masks

    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, action_masks: th.Tensor) -> Distribution:
        action_logits = self.action_net(latent_pi)
        # logger.info('distribution before: {}'.format(action_logits.numpy()))
        action_logits = th.mul(action_logits, th.Tensor(action_masks))
        # logger.info('distribution after: {}'.format(action_logits.numpy()))
    ...
    def predict(
        ...
        with th.no_grad():
            ...
            actions, states = self._predict(observation,
                                            lstm_states=states,
    ...
    def predict_prob(
        ...
        with th.no_grad():
            ...
            states = th.tensor(state[0]).float().to(self.device), th.tensor(state[1]).float().to(self.device)
            episode_starts = th.tensor(episode_start).float().to(self.device)
            distribution, lstm_states = self.get_distribution(observation, states, episode_starts, action_masks)
            return distribution.distribution.logits.numpy()[0]
